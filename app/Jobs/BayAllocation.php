...
class BayAllocation implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach ($flights as $ac) {
            ...
            if ($dist['ICAO'] !== null && (float) $ac->speed < AirportFlightState::STATIONARY_SPEED_KTS) {
        ...
        $clearBays = Bays::where('status', 2)->where('clear', 1)->with('FlightInfo')->get();
    ...
    private function airportDistance($lat, $lon, $airports)
    {
        ...
        foreach ($airports as $icao => $airport) {
            $distances[$icao] = $state->distanceNm($lat, $lon, $airport->lat, $airport->lon);
        }
        asort($distances);
        $closestDistance = reset($distances);
        $distances['ICAO'] = $closestDistance !== false && $closestDistance < AirportFlightState::LOCAL_AIRPORT_RADIUS_NM
            ? key($distances)
            : null;
    ...
    private function BayDistanceChecker(float $lat1, float $lon1, float $lat2, float $lon2): float
    {
        return app(AirportFlightState::class)->distanceMeters($lat1, $lon1, $lat2, $lon2);
    }
}
